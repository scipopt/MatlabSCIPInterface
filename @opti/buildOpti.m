...
if(~any(strcmpi(solver,{'auto','scip'}))) %allow scip or NLP solver to solve any QC constraint form
    %Check for equality
    if(rl == ru)
        error('Quadratic Constraint %d is an equality constraint. Please specify SCIP to solve this problem.',i);
    end
...
if(any(e < 0)) %assuming symmetric...
    if(strcmpi(solver,'cplex'))
        ...
        error('Quadratic Constraint Q matrix in cell %d (%s) is not positive semi-definite! Please specify SCIP to solve this problem.',i,str);       
    end
end 
