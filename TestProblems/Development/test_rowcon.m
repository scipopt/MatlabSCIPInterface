...
nle = [1 0]'; % (>=, ==)
%Setup Options
opts = optiset('solver','scip','warnings','on','display','iter');
%Build & Solve
x0 = [1 5 5 1]';
Opt = opti('obj',obj,'grad',grad,'nlmix',nlcon,nlrhs,nle,'nljac',nljac,'bounds',lb,ub,'options',opts)
[x,fval,exitflag,info]= solve(Opt,x0)

% TODO Commented out since it ran endlessly or looped
...
% 
% %Setup Options
% opts = optiset('solver','scip','display','iter','maxiter',1e4);
% %Build & Solve
% x0 = [ 2.5 0.5 2 -1 2.5 ];
...
xval = [ 2.5 0.5 2 -1 1.5 ];

prob = optiprob('nl',nlcon,cl,cu,'xval',xval)
prob = nrow2mix(prob)
...
prob = optiprob('nl',nlcon,cl,cu,'nljac',nljac,'xval',xval)
prob = nrow2mix(prob)
...
prob = optiprob('nl',nlcon,cl,cu,'nljac',nljac,'nljacstr',nljacstr,'xval',xval)
prob = nrow2mix(prob)
...
clc
obj=@(x) (x)^2;
A=1; ru=1; rl=-1; xtype='I'; xval=1;
opt=opti('obj',obj,'xtype',xtype,'xval',xval,'lin',A,rl,ru)
opt.prob.sizes
solve(opt,xval)
...
clc
obj=@(x) (x)^2;
A=[1;-1];  b=[1;1];  xtype='I'; xval=1;
opt=opti('obj',obj,'xtype',xtype,'xval',xval,'ineq',A,b); 
opt.prob.sizes
solve(opt,xval)
